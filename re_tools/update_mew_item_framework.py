"""
update_mew_item_framework.py

Scans a Mewgenics executable for byte signatures and automatically updates the
RVA #define values in MewItemFramework.h.

Run this after a game update to keep the framework compatible:

    python update_mew_item_framework.py path\to\Mewgenics.exe src\MewItemFramework.h

Options:
    --signatures  Path to mew_item_framework_signatures.json
                  (defaults to the file next to this script)
    --dry-run     Print resolved RVAs without modifying the header
    --no-backup   Skip creating a .bak copy before rewriting the header
"""
...
def find_pattern(buffer: bytes, pattern: bytes, mask: list[bool]) -> list[int]:
    ...
    while True:
        ...
        if start >= 0 and start + pattern_length <= len(buffer):
            is_match: bool = all(buffer[start + i] == pattern[i] for i in fixed_indices)
...
def find_unique_pattern(pe_image: PeImage, symbol_name: str, symbol_config: dict[str, Any]) -> tuple[Section, int]:
    ...
    search_section_names: set[str] = set(str(v) for v in symbol_config.get("search_sections", []))
    ...
    if len(found) > 1:
        locations: str = ", ".join(
            f"{section.name}+0x{relative_offset:X}" for section, relative_offset in found[:8]
        )
...
def update_header_text(header_text: str, resolved_values: dict[str, int]) -> tuple[str, list[str]]:
    ...
    for symbol_name, value in resolved_values.items():
        ...
        def replace(match: re.Match[str], _name: str = symbol_name, _value: int = value) -> str:
            updated_symbols.append(_name)
            return (
                f"{match.group(1)}"
                f"{format_define_value(match.group(2), _value)}"
                f"{match.group(3)}"
                f"{match.group(4)}"
            )

        header_text, count = pattern.subn(replace, header_text, count=1)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Update MewItemFramework.h RVAs from Mewgenics byte signatures."
    )
    parser.add_argument("exe_path", type=Path, help="Path to the updated Mewgenics executable.")
    parser.add_argument("header_path", type=Path, help="Path to MewItemFramework.h.")
    parser.add_argument(
        "--signatures",
        type=Path,
        default=Path(__file__).with_name("mew_item_framework_signatures.json"),
        help="Path to mew_item_framework_signatures.json (default: next to this script).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Resolve symbols and print results without modifying the header.",
    )
    parser.add_argument(
        "--no-backup",
        action="store_true",
        help="Do not create a .bak copy before rewriting the header.",
    )
    args = parser.parse_args()

    pe_image = PeImage(args.exe_path)
    signature_config = load_signatures(args.signatures)
    resolved_values: dict[str, int] = {}

    for symbol_name, symbol_config_any in signature_config["symbols"].items():
        result = resolve_symbol(pe_image, symbol_name, dict(symbol_config_any))
    ...
    if not args.no_backup:
        backup_path = args.header_path.with_suffix(args.header_path.suffix + ".bak")
        shutil.copy2(args.header_path, backup_path)
        print(f"Backup written: {backup_path}")

    args.header_path.write_text(updated_text, encoding="utf-8", newline="")
    print(f"Updated {len(updated_symbols)} symbol(s) in {args.header_path}")
    return 0
